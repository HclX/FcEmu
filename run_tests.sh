...
echo -e "\n${YELLOW}[Step 1/4] Running Rust Core Unit Tests...${NC}"
cargo test

# Step 2: Ensure headless CLI test binary is compiled
echo -e "\n${YELLOW}[Step 2/4] Compiling Headless CLI Test Binary...${NC}"
cargo build --bin headless

# Step 3: Run Blargg Test Suite Verification Harness
echo -e "\n${YELLOW}[Step 3/4] Executing Blargg Automated Test Suite...${NC}"

# Auto-download check for missing official blargg test ROMs
if [ ! -f "tests/roms/instr_official_only.nes" ] || [ ! -f "tests/roms/cpu_dummy_writes.nes" ] || [ ! -f "tests/roms/branch_timing.nes" ]; then
    echo -e "${YELLOW}Official blargg test ROM files are missing. Running tests/download_test_roms.sh automatically...${NC}"
...
echo -e "\n${YELLOW}[Step 4/4] Executing Nova the Squirrel Visual Golden Harness...${NC}"
